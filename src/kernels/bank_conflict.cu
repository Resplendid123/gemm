...
// ========== Stage 4: Bank Conflict GEMM ==========
template <int BM, int BN, int BK, int TM, int TN>
__global__ void bank_conflict_gemm_kernel(float *C, const float *A, const float *B,
                                          int M, int N, int K,
...
// ========== 包装宏，统一启动方式 ==========
#define LAUNCH_BANK_KERNEL(BM, BN, BK, TM, TN)                                 \
    do                                                                         \
    {                                                                          \
        dim3 block(BN / TN, BM / TM);                                          \
        dim3 grid((N + BN - 1) / BN, (M + BM - 1) / BM);                       \
        bank_conflict_gemm_kernel<BM, BN, BK, TM, TN>                          \
                              ...
                              cudaStream_t stream, int config)
{
    ...
    {
    case 1:
        LAUNCH_BANK_KERNEL(64, 64, 8, 4, 4); // Config A: 小块配置
        break;
    case 2:
        LAUNCH_BANK_KERNEL(64, 128, 8, 4, 8); // Config B: 大列配置
        break;
    case 3:
        LAUNCH_BANK_KERNEL(128, 128, 8, 8, 8); // Config C: 大块配置 BK=8
        break;
    case 4:
        LAUNCH_BANK_KERNEL(128, 128, 16, 8, 8); // Config D: 大块配置 BK=16
        break;
    default:
        LAUNCH_BANK_KERNEL(64, 64, 8, 4, 4);
        break;
    }
